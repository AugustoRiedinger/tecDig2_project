...
Para la síntesis del código se utilizan las siguientes reglas:
 ...
 + Funciones creadas por el usuario se identifican en mayúsculas. Ej.: =void MY_FUNCTION()=.
 + Variables globales o definiciones creadas por el usuario se identifican en minúsculas. Ej.: =int myGlobalVar = x=.
 + Variables locales creadas por el usuario se identifican con la primer letra en mayúscula y el resto en minúsculas. Ej.: =int MyLocalVar = x=.
